...
- Start with named read-only tools over YNAB plans/accounts/categories/months/transactions.
- Keep the architecture ready for explicit write tools without exposing generic mutation.
...
### Phase 2 — Claude-web-compatible read-only prototype

- Goal: Implement and test a TypeScript Streamable HTTP MCP server with the initial read-only YNAB tool slice.
- Why now: This validates real Claude web connector behavior before expanding tool coverage.
...
### Phase 4 — Write tools and optional escape hatch

- Goal: Add explicit write tools, starting with the safest high-value action, and optionally a constrained read-only endpoint escape hatch.
- Why later: Write operations affect financial records and need separate confirmation, annotations, and tests.
